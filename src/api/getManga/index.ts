...
const getManga = (genre: GenreFilter) => {
	return axios.get(`https://api.jikan.moe/v4/manga?genres=${genre}`).then(response => {
		const formattedResponse: GetMangaSearchResponse[] = response.data.data.map((manga: MangaSearchRawResponse) => {
...
export const getSingleManga = (animeTitle: string) => {
	return axios.get(`https://api.jikan.moe/v4/manga?q=${encodeURIComponent(animeTitle)}`).then(response => {
